...
class M_project extends CI_Model
{
    ...
    function getThreeDaysPlanning(){
        $query = $this ->db->query("SELECT * FROM project_plannings WHERE status ='1' OR status ='2' AND actual_delivery_date BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 10 DAY)");
    ...
    function TotalPlan($level,$type){
        $query = $this ->db->query("SELECT COUNT(drw_id) as tPlan FROM project_drawings as pd, project_plannings as pp WHERE pp.panel_number = pd.drw_id AND pd.level = '".$level."' AND pd.panel_type = '".$type."'");
    ...
    function TotalDrawing($level,$type){
        $query = $this ->db->query("SELECT COUNT(drw_id) as tPlan FROM getdrawinhinfo WHERE panel_type = '".$type."' AND level = '".$level."'");
    ...
    function TotalApproveDrawing($level,$type){
        $query = $this ->db->query("SELECT COUNT(drw_id) as taPlan FROM getdrawinhinfo WHERE panel_type = '".$type."' AND level = '".$level."' AND status='1'");
    ...
    function totalProducePlan($level,$type,$status){
        $query = $this ->db->query("SELECT COUNT(drw_id) as tPro FROM project_drawings as pd, project_plannings as pp WHERE pp.panel_number = pd.drw_id AND pp.status = '".$status."' AND pd.level = '".$level."' AND pd.panel_type = '".$type."'");
