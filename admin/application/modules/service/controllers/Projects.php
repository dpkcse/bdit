...
class Projects extends Web_Controller
{
    ...
    function DrawingAction($param1 = '', $param2 = '')
    {
        if ($this->session->userdata('admin_logged_in')) {
            ...
            if ($param1 == 'do_update') {
                ...
                if($this->input->post('chkActive') == ""){
                    $status = '0';
                }else if($this->input->post('chkActive') == "0"){
                    $status = '0';
                }else{
                    $status = '1';
                }
    ...
    public function getDrawingforDeliver(){
        if ($this->session->userdata('admin_logged_in')) {
            ...
            $jsonData['drawings'] = $this->db
                                         -> group_by('panel_type')
                                         ->get_where('project_drawings', array('status' => '1','level'=>$value))->result_array();
    ...
    public function getSelectDrawingList(){
        if ($this->session->userdata('admin_logged_in')) {
            ...
            $result = $this->db->get_where('getalldrawigfromdrawingtbl', array('panel_type' => $this->input->post('value')))->result();
